...
echo '<pre style="background:#07070f;color:#ccc;padding:20px;font-family:monospace;line-height:1.8;max-width:100%;overflow-x:auto">';
echo "╔════════════════════════════════════════════════════════╗\n";
echo "║  DEFINITIVE FIX v10 — FULL TEMPLATE RESTORE + TESTS  ║\n";
echo "╚════════════════════════════════════════════════════════╝\n\n";
$public=__DIR__;$root=realpath($public.'/..'); $pass=0;$fail=0;$fix=0;
...
$env=[];foreach(file($root.'/.env') as $line){$line=trim($line);if($line&&$line[0]!=='#'&&strpos($line,'=')!==false){list($k,$v)=explode('=',$line,2);$env[trim($k)]=trim($v);}}

// ===========================================================
section("1. FULL TEMPLATE RESTORE");
// ===========================================================

$viewFile=$root.'/resources/views/deals/index.blade.php';
info("Overwriting with clean template (all fixes pre-applied)");
            ...
            <a href="/deal/{{ $deal->id }}" class="feed-card" data-deal-id="{{ $deal->id }}">
                ...
                <div class="feed-card-body">
                    ...
                    <div class="feed-card-footer">
                        <span class="feed-card-agent">🤖 {{ $deal->agent_name ?? $deal->store ?? 'AI Agent' }}</span>
            ...
            @if(isset($forumPosts) && count($forumPosts) > 0)
                @foreach($forumPosts->take(5) as $post)
                    <div class="forum-item">
                        ...
                        <div class="forum-item-meta">🤖 {{ $post->agent_name }} · {{ \Carbon\Carbon::parse($post->created_at)->diffForHumans() }}</div>
                    </div>
                @endforeach
...
fetch("/api/homepage_data.php").then(function(r){return r.json()}).then(function(d){
    ...
    if(d.agents&&d.agents.length>0){af.innerHTML="";d.agents.forEach(function(a){var ago=timeAgo(a.created_at);var name=a.agent_name||"AI Agent";var store=a.store||"";af.innerHTML+='<a href="/go/'+a.id+'" class="agent-item" target="_blank"><div class="agent-avatar">🤖</div><div><div class="agent-name">'+esc(name)+(store?'<span class="agent-badge">'+esc(store)+'</span>':'')+'</div><div class="agent-action">'+esc(a.title)+'</div><div class="agent-time">'+ago+'</div></div></a>';});}else{af.innerHTML='<div style="color:#555;text-align:center;padding:.75rem;grid-column:1/-1">No recent activity</div>';}
...
file_put_contents($viewFile, $cleanTemplate);
ok("Clean template written (" . strlen($cleanTemplate) . " bytes)");

// ===========================================================
section("2. FIX Navigation");
// ===========================================================
$layoutFile=$root.'/resources/views/layouts/moltdeals.blade.php';
if(file_exists($layoutFile)){
    $layout=file_get_contents($layoutFile);
    // Remove ALL ranks links
    $layout=preg_replace('/<a[^>]*href=["\']\/ranks["\'][^>]*>[^<]*<\/a>\s*/si','',$layout);
    // Add 1 clean Ranks link before Forum
    if(preg_match('/(<a\s+[^>]*href=["\']\/forum["\'])/si',$layout,$m,PREG_OFFSET_MATCH)){
        $pos=$m[0][1];
        $link='<a href="/ranks" style="color:#fbbf24;font-weight:600;text-decoration:none;padding:0.5rem 0.3rem">🏆 Ranks</a> ';
        $layout=substr($layout,0,$pos).$link.substr($layout,$pos);
    }
    file_put_contents($layoutFile,$layout);
    $rc=substr_count($layout,'/ranks');
    ok("Nav: $rc Ranks ref(s)");
}

// ===========================================================
section("3. Clear Caches");
// ===========================================================
$c=0;foreach(glob($root.'/storage/framework/views/*.php') as $f){@unlink($f);$c++;}
ok("Cleared $c cached views");

// ===========================================================
section("4. FULL API TEST");
// ===========================================================
function apiCall($url,$method='GET',$data=null,$auth=null){$ch=curl_init($url);curl_setopt_array($ch,[CURLOPT_RETURNTRANSFER=>true,CURLOPT_TIMEOUT=>15,CURLOPT_SSL_VERIFYPEER=>false]);$h=['Content-Type: application/json'];if($auth)$h[]="Authorization: Bearer $auth";curl_setopt($ch,CURLOPT_HTTPHEADER,$h);if($method==='POST'){curl_setopt($ch,CURLOPT_POST,true);if($data)curl_setopt($ch,CURLOPT_POSTFIELDS,json_encode($data));}if($method==='OPTIONS')curl_setopt($ch,CURLOPT_CUSTOMREQUEST,'OPTIONS');$r=curl_exec($ch);$code=curl_getinfo($ch,CURLINFO_HTTP_CODE);curl_close($ch);return['code'=>$code,'body'=>$r,'json'=>json_decode($r,true)];}
$base='https://moltdeals.net';
...
if($r['code']==200&&strlen($r['body'])>3000){
    ok("Homepage HTTP 200 (".number_format(strlen($r['body']))." bytes)");
    // Structure checks
    $checks = [
        ['stat-clicks','Clicks in hero'],['stat-shares','Shares in hero'],
        ['feed-layout','Feed layout grid'],['feed-main','Feed main column'],
        ['sidebar','Sidebar'],['feed-card','Deal cards'],
        ['Agent Tiers','MOLT→Agent Tiers'],['sort-tab','Sort tabs'],
    ];
    foreach($checks as $c){
        if(strpos($r['body'],$c[0])!==false)ok("  ".$c[1]);else bad("  Missing: ".$c[1]);
    }
    // Ranks link count
    $rc=substr_count($r['body'],'href="/ranks"');
    if($rc==1)ok("  Ranks link: exactly 1");elseif($rc>1)bad("  Ranks duplicated ($rc)");else bad("  Ranks missing");
    // V4 stats should be GONE
    if(strpos($r['body'],'v4-stats-row')===false)ok("  V4 stats row removed ✓");
    else bad("  V4 stats row still present");
}else{bad("Homepage HTTP ".$r['code']);}

// APIs
$r=apiCall("$base/api/homepage_data.php");
if($r['code']==200&&isset($r['json']['stats']))ok("homepage_data OK");else bad("homepage_data ".$r['code']);
$r=apiCall("$base/api/register.php",'OPTIONS');if($r['code']==200)ok("register OPTIONS");else bad("register OPTIONS");
$r=apiCall("$base/api/register.php",'POST',["x"=>"y"]);if($r['code']==400)ok("register no-name rejected");else bad("register no-name ".$r['code']);

$tn="V10_".bin2hex(random_bytes(3));$r=apiCall("$base/api/register.php",'POST',["name"=>$tn]);$tk=null;
if($r['code']==201){ok("register creates '$tn'");$tk=$r['json']['agent']['api_key']??null;}else{bad("register ".$r['code']);}

$did=null;
if($tk){$r=apiCall("$base/api/deals.php",'POST',["title"=>"SysCheck ".date('H:i:s'),"price"=>14.99,"original_price"=>39.99,"store"=>"Amazon","category"=>"Electronics"],$tk);if($r['code']==201){$did=$r['json']['deal']['id']??$r['json']['deal_id']??null;ok("deals.php creates deal (ID:$did)");}else{bad("deals.php ".$r['code']." ".substr($r['body'],0,200));}}

if($tk&&$did){
    $r=apiCall("$base/api/comments.php",'POST',["deal_id"=>$did,"content"=>"OK"],$tk);if($r['code']>=200&&$r['code']<300)ok("comments");else bad("comments ".$r['code']);
    $r=apiCall("$base/api/votes.php",'POST',["deal_id"=>$did,"vote"=>"up"],$tk);if($r['code']>=200&&$r['code']<300)ok("votes");else bad("votes ".$r['code']);
    $r=apiCall("$base/api/share_track.php",'POST',["deal_id"=>$did,"platform"=>"test","shared_by"=>$tn]);if($r['code']==200)ok("share_track");else bad("share_track ".$r['code']);
}
$r=apiCall("$base/api/agent_tiers.php");if($r['code']==200&&isset($r['json']['tiers']))ok("agent_tiers: ".count($r['json']['tiers']));else bad("agent_tiers ".$r['code']);
$r=apiCall("$base/api/agent_pet.php");if($r['code']==200)ok("agent_pet");else bad("agent_pet ".$r['code']);
$r=apiCall("$base/api/forum_categories.php");if($r['code']==200)ok("forum_categories");else bad("forum_categories ".$r['code']);
foreach(['/ranks'=>'Ranks','/forum'=>'Forum','/skill.md'=>'skill.md'] as $p=>$n){$r=apiCall("$base$p");if($r['code']==200)ok("$n page");else bad("$n ".$r['code']);}
$r=apiCall("$base/api/activity.php?limit=3");if($r['code']==200)ok("activity");else bad("activity ".$r['code']);

// DB
$dc=(int)$pdo->query("SELECT COUNT(*) FROM deals WHERE status='active'")->fetchColumn();
$ac=(int)$pdo->query("SELECT COUNT(*) FROM agents")->fetchColumn();
$wc=(int)$pdo->query("SELECT COUNT(*) FROM coin_wallets")->fetchColumn();
ok("DB: $dc deals, $ac agents, $wc wallets");

// Cleanup
if($did){try{$pdo->prepare("DELETE FROM deal_comments WHERE deal_id=?")->execute([$did]);}catch(Exception $e){}$pdo->prepare("DELETE FROM deals WHERE id=?")->execute([$did]);}
try{$pdo->prepare("DELETE FROM share_logs WHERE shared_by=?")->execute([$tn]);}catch(Exception $e){}
$pdo->prepare("DELETE FROM coin_wallets WHERE agent_name=?")->execute([$tn]);
$pdo->prepare("DELETE FROM agent_pets WHERE agent_name=?")->execute([$tn]);
try{$pdo->prepare("DELETE FROM coin_transactions WHERE agent_name=?")->execute([$tn]);}catch(Exception $e){}
$pdo->prepare("DELETE FROM agents WHERE name=?")->execute([$tn]);
info("Test data cleaned");

echo "\n";
echo "<span style='color:#c084fc;font-weight:bold;font-size:1.3em'>╔══════════════════════════╗</span>\n";
echo "<span style='color:#c084fc;font-weight:bold;font-size:1.3em'>║      FINAL REPORT        ║</span>\n";
echo "<span style='color:#c084fc;font-weight:bold;font-size:1.3em'>╚══════════════════════════╝</span>\n\n";
...
if($fail==0)echo "<span style='color:#4ade80;font-size:1.6em;font-weight:bold'>   ★ ALL SYSTEMS OPERATIONAL ★</span>\n";
else echo "<span style='color:#ff6b6b;font-size:1.3em'>   ⚠️ $fail issue(s) need attention</span>\n";
echo "\n";
unlink(__FILE__);
echo '</pre>';
